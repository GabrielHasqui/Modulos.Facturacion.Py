...
import os
from utils.formatters import formatear_dinero
...
class Sale(Icalculo):
    ...
    def print_invoice(self,company):
        ...
        for det in self.sale_detail:
            print(blue_color+f"{det.id:5} {det.product.descrip:10} {formatear_dinero(det.preci):>10} {det.quantity:8} {formatear_dinero(det.preci*det.quantity):>12}")  
        print(green_color+"*"*70+reset_color)    
        print(purple_color+" "*23,"Subtotal:  ",formatear_dinero(self.subtotal))
        print(" "*23,"Descuento: ",formatear_dinero(self.discount))
        print(" "*23,"IVA:       ",formatear_dinero(self.iva))
        print(" "*23,"Total:     ",formatear_dinero(self.total)+reset_color)  
